...
We can create a fully reproducible docker container with the data preparation and analysis for a specific project using `admin/create_docker.R`  
...
When a project is ready to share, you can create a self-contained docker container:  
...
# Create a Dockerfile in the project's main folder and builds the docker container
create_docker_container(PID = PID)

```

This will create a file called pid999.tar.zip. This file contains a docker image with all the necessary to rebuild the data preparation and analysis for the project 999.

### Run container

You will be able to run the data preparation and analysis project inside a docker container, ensuring reproducibility. The output will be in Downloads/jsPsychHelpeR/pid[PID]/ after a couple of minutes.  
...
After copying the file pid999.tar.zip to your computer, run the following command in a cmd shell:  

```{r, echo=FALSE}
PID = 999

cat(paste0("tar -xf pid", PID, ".tar.zip & ", 
       "docker load input - & ",
