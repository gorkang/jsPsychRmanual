...
-   Edit `parameters_monkeys_minimal`. For example, launch participants 1 to 5 to the `protocol999` protocol locally:

    -   `parameters_monkeys_minimal = list(uid = 1:5, local_folder_tasks = "~/Downloads/protocol999")`  
    
    -   If you are on Windows, `local_folder_tasks` value should be something similar to `C:/Users/myusername/Downloads/protocol999`  

-   Run Monkeys!: `targets::tar_make()`


If the protocol was running from a loccal folder, the Monkey's responses will be copied to a subfolder `.data/` inside the `local_folder_tasks`. In the example above, `~/Downloads/protocol999/.data`. If the protocol was running on the server, the data will be in the protocols' `.data/` folder inside the server.  
